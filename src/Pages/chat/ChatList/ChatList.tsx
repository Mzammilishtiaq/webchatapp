...
// const userchat = userchatlist.map((item:any)=>{})
    ...
    //   userChatsData.chats[0].MessageReadSeen = true;
    ...
    <div className="gap-4 flex flex-col relative top-0 left-0 right-0 bottom-0">
      <div className=" flex items-center justify-between gap-1">
        ...
        <div onClick={() => setUserinfodetail(pre => !pre)} className="flex relative cursor-pointer">
                ...
                <div className={`w-5 h-5 rounded-full absolute left-10 top-10 ${currentUser?.status == 'online'? 'bg-green-500':'bg-red-500'}`}></div>
              </div>
          ...
          {userchatlist?.map((chat: any) => (
            <div className='flex items-center gap-5 cursor-pointer border-b-2 py-3' key={chat.chatid} onClick={() => handleselect(chat)}>
              <div className="flex relative">
                ...
                <div className={`w-5 h-5 rounded-full absolute left-10 top-10 ${chat?.user?.status == 'online' ? 'bg-green-500':'bg-red-500'}`}></div>
              </div>
              <div className="">
